...
require("lazy").setup({
	{
		...
		config = function()
			require("mdpubs").setup({
				...
				-- Your API key (get this from https://mdpubs.com)
...
-- Example workflow:
-- 1. Get your API key from 'https://mdpubs.com'
-- 2. Copy the api_key from the response
-- 3. Update the api_key in your config above

-- 4. To sync files with the API, add `mdpubs` frontmatter to your markdown files:
--
...
-- 6. Files without 'mdpubs:' in frontmatter will be ignored (not synced)
